import template from './nfx-analytics-index-sales-by-quarter-invoice.html.twig';

const { Component, Mixin } = Shopware;

Component.register('nfx-analytics-index-sales-by-quarter-invoice', {
    template,
    ...
    computed: {
        ...
        chartOptions() {
            return {
                title: {
                    text: this.$tc('nfx-analytics.view.salesByQuarterInvoice.titleChart')
                },
                stroke: {
                    curve: 'smooth'
                },
                ...
                yaxis: [
                    {
                        title: {
                            text: this.$tc('nfx-analytics.view.sumColumn')
                        },
                        min:0,
                        forceNiceScale: true,
                        labels:{
                        ...
                        opposite: true,
                        title: {
                            text: this.$tc('nfx-analytics.view.countColumnShort')
                        },
                        labels:{
        ...
        chartSeriesData() {
            let chartData = this.defaultChartSeriesData(this.seriesData, 'date', 'sumNet',this.$tc('nfx-analytics.view.sumColumn'), true);
            if (Array.isArray(chartData)) {
                chartData.push(this.defaultChartSeriesData(this.seriesData, 'date', 'count',this.$tc('nfx-analytics.view.countColumnShort'), true)[0]);
            }
        ...
        getGridColumns() {
            return [{
                property: 'date',
                dataIndex: 'date',
                label: this.$tc('nfx-analytics.view.salesByQuarterInvoice.dateColumn'),
                allowResize: false,
                primary: true,
                inlineEdit: false,
                width: '100px'
    ...
    methods: {
        createdComponent() {
            ...
            if (this.activeStatistic.name === 'sales_by_quarter_invoice' && this.activeStatistic.pathInfo) {
