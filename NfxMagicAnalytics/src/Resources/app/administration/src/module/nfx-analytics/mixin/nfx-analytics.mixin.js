...
Mixin.register('nfx-analytics', {
    ...
    data() {
        return {
            orderStoreKey: 'grid.filter.nfx.analytics.order',
            productStoreKey: 'grid.filter.nfx.analytics.product',
    ...
    methods: {
        ...
        getCsvDownloadUrl(fileName, fileSize) {
            return `${Shopware.Context.api.apiResourcePath}/nfx/analytics/download?fileName=${fileName}&fileSize=${fileSize}`;
        },
        ...
        onColumnSort(column) {
            ...
            if (this.gridData.length > 0 && this.gridData[this.gridData.length -1].name === this.$tc('nfx-analytics.data.others')) {
