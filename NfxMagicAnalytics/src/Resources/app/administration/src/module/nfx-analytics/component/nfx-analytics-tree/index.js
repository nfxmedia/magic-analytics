import template from './nfx-analytics-tree.html.twig';
import './nfx-analytics-tree.scss';

const { Mixin, Component } = Shopware;
const { Criteria } = Shopware.Data;
const USER_CONFIG_KEY = 'nfx.analytics.favorites';

Component.register('nfx-analytics-tree', {
    template,
    ...
    methods: {
            ...
            this.statisticsRepository.search(statisticsCriteria).then((result) => {
                ...
                if (result.length > 0) {
                    ...
                    treeItems.push({
                        data: { active:true },
                        id: i+1,
                        name: this.$tc('nfx-analytics.general.favorites'),
                ...
                this.$nextTick(() => {
                    ...
                    if (treeItems[0].data.name === 'quick_overview') {
                        this.$emit('nfx-statistic-selection', treeItems[0]);
                    }
                    this.isLoading = false;
                });
            }).catch((err) => {
        ...
        changeSelectedStatistic(element) {
            ...
            if (element.parentId || element.data.data.name === 'quick_overview') {
                this.$emit('nfx-statistic-selection', element.data);
