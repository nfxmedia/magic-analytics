import template from './nfx-analytics-product.html.twig';
import './nfx-analytics-product.scss';

const { Component, Context, Mixin } = Shopware;
const { Criteria } = Shopware.Data;

Component.register('nfx-analytics-product', {
    template,
    ...
    data() {
        return {
            ...
            sortDirection: 'DESC',
            sortBy: 'sum',
            storeKey: 'grid.filter.nfx.analytics.order',
    ...
    methods: {
        ...
        async getGridData() {
            ...
            try {
                ...
                httpClient.post('/nfx/analytics/getProductsGridData',
                    { parameters },
        ...
        getProductColumns() {
            return [{
                ...
                property: 'gross',
                dataIndex: 'gross',
                label: this.$tc('nfx-analytics.product.sales') + this.$tc('nfx-analytics.view.gross'),
                allowResize: true,
                align: 'right',
                inlineEdit: false,
            }, {
                property: 'net',
                dataIndex: 'net',
                label: this.$tc('nfx-analytics.product.sales') + this.$tc('nfx-analytics.view.net'),
                allowResize: true,
                align: 'right',
                inlineEdit: false,
            }, {
                property: 'gprice',
                dataIndex: 'gprice',
                label: this.$tc('nfx-analytics.product.avPrice') + this.$tc('nfx-analytics.view.gross'),
                allowResize: true,
                align: 'right',
                inlineEdit: false,
            }, {
                property: 'nprice',
                dataIndex: 'nprice',
                label: this.$tc('nfx-analytics.product.avPrice') + this.$tc('nfx-analytics.view.net'),
                allowResize: true,
                align: 'right',
                inlineEdit: false,
            }, {
        ...
        exportCSV() {
            ...
            if (Array.isArray(products) && products.length > 0) {
                ...
                products.forEach((product) => {
                    ...
                    item[this.$tc('nfx-analytics.product.sales') + this.$tc('nfx-analytics.view.gross')] = Shopware.Filter.getByName('currency')(product.gross, this.systemCurrency.isoCode, 2);
                    item[this.$tc('nfx-analytics.product.sales') + this.$tc('nfx-analytics.view.net')] = Shopware.Filter.getByName('currency')(product.net, this.systemCurrency.isoCode, 2);;
                    item[this.$tc('nfx-analytics.product.avPrice') + this.$tc('nfx-analytics.view.gross')] = Shopware.Filter.getByName('currency')(product.gprice, this.systemCurrency.isoCode, 2);
                    item[this.$tc('nfx-analytics.product.avPrice') + this.$tc('nfx-analytics.view.net')] = Shopware.Filter.getByName('currency')(product.nprice, this.systemCurrency.isoCode, 2);
                ...
                if (data.length > 0) {
                    httpClient.post('nfx/analytics/csvExport',
                        {data},
                        {headers: {Authorization: `Bearer ${loginService.getToken()}`,}}
                    ).then((response) => {
