import template from './nfx-analytics-index-sales-by-account-types.html.twig';

const { Component, Mixin } = Shopware;

Component.register('nfx-analytics-index-sales-by-account-types', {
    template,
    ...
    computed: {
        ...
        chartOptions() {
            return this.pieChartOptions('nfx-analytics.view.salesByAccountTypes.titleChart', this.systemCurrency);
        },

        chartSeriesData() {
            return this.defaultChartSeriesData(this.gridData, 'name', 'sum', 'Umsatz', false, true);
        },
        ...
        getGridColumns() {
            return [{
                property: 'name',
                dataIndex: 'name',
                label: this.$tc('nfx-analytics.view.salesByAccountTypes.nameColumn'),
                allowResize: false,
                primary: true,
                inlineEdit: false,
                width: '100px'
    ...
    methods: {
        createdComponent() {
            ...
            if (this.activeStatistic.name === 'sales_by_account_types' && this.activeStatistic.pathInfo) {
                httpClient.post(this.activeStatistic.pathInfo,
                    ...
                    if (response.data !== undefined && response.data['success'] === true && response.data['gridData']) {
                        response.data['gridData'].forEach((item) => {
                            item.name = this.$tc(item.name);
                        })
