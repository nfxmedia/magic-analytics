import template from './nfx-analytics-index-sales-by-campaign.html.twig';

const { Component, Mixin } = Shopware;

Component.register('nfx-analytics-index-sales-by-campaign', {
    template,
    ...
    computed: {
        ...
        chartOptions() {
            return this.pieChartOptions('nfx-analytics.view.salesByCampaign.titleChart', this.systemCurrency);
        },
        ...
        getGridColumns() {
            return [{
                property: 'name',
                dataIndex: 'name',
                label: this.$tc('nfx-analytics.view.salesByCampaign.nameColumn'),
                allowResize: false,
                primary: true,
                inlineEdit: false,
                width: '100px'
                ...
                property: 'sum',
                dataIndex: 'sum',
                label: this.$tc('nfx-analytics.view.sumColumn') + this.$tc('nfx-analytics.view.' + this.grossOrNet),
                allowResize: false,
                align: 'right',
                inlineEdit: false,
                width: '100px'
    ...
    methods: {
        createdComponent() {
            ...
            if (this.activeStatistic.name === 'sales_by_campaign' && this.activeStatistic.pathInfo) {
        ...
        setLabels(data) {
            for (let i = 0; i < data.length; i++) {
                ...
                } else if (data[i].name === 'nfx-analytics.view.noCampaign') {
                    data[i].name = this.$tc('nfx-analytics.view.noCampaign');
                }
            }
            return data;
        },
