import template from './nfx-analytics-order.html.twig';

const { Component, Context, Mixin } = Shopware;
const { Criteria } = Shopware.Data;

Component.register('nfx-analytics-order', {
    template,
    ...
    methods: {
        ...
        getOrderColumns() {
            return [{
                ...
                property: 'amountTotal',
                label: this.$tc('sw-order.list.columnAmount') + this.$tc('nfx-analytics.view.gross'),
                align: 'right',
                allowResize: true,
            }, {
                property: 'amountNet',
                label: this.$tc('sw-order.list.columnAmount') + this.$tc('nfx-analytics.view.net'),
                align: 'right',
                allowResize: true,
            }, {
                //Profit
                property: 'id',
                label: this.$tc('nfx-analytics.order.profitColumn') + this.$tc('nfx-analytics.view.net'),
                allowResize: true,
                visible: true,
            }, {
        ...
        exportCSV() {
            if (Array.isArray(this.orders) && this.orders.length > 0) {
                ...
                this.orders.forEach((order) => {
                    ...
                    item[this.$tc('sw-order.list.columnAmount') + this.$tc('nfx-analytics.view.gross')] = Shopware.Filter.getByName('currency')(order.amountTotal, order.currency.isoCode, 2);
                    item[this.$tc('sw-order.list.columnAmount') + this.$tc('nfx-analytics.view.net')] = Shopware.Filter.getByName('currency')(order.amountNet, order.currency.isoCode, 2);
