import template from './nfx-analytics-entity-multi-select.html.twig';

const { Component } = Shopware;
const { Criteria } = Shopware.Data;

Component.extend('nfx-analytics-entity-multi-select', 'sw-entity-multi-id-select', {
    template,
    ...
    methods: {
        createdComponent() {
            ...
            if (this.name === 'NfxModulAnalytics.config.blacklistedOrderStates') {
                this.criteria.addAssociation('stateMachine');
                this.criteria.addFilter(Criteria.equals('stateMachine.technicalName', 'order.state'));
            } else if (this.name === 'NfxModulAnalytics.config.blacklistedTransactionStates') {
                this.criteria.addAssociation('stateMachine');
                this.criteria.addFilter(Criteria.equals('stateMachine.technicalName', 'order_transaction.state'));
            } else if (this.name === 'NfxModulAnalytics.config.blacklistedDeliveryStates') {
                this.criteria.addAssociation('stateMachine');
                this.criteria.addFilter(Criteria.equals('stateMachine.technicalName', 'order_delivery.state'));
            } else if (this.name === 'NfxModulAnalytics.config.defaultSaleschannels' || this.name === 'NfxModulAnalytics.config.dashboardSaleschannels') {
