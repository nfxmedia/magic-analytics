import template from './nfx-analytics-index-sales-all.html.twig';

const { Component, Mixin } = Shopware;

Component.register('nfx-analytics-index-sales-all', {
    template,
    ...
    computed: {
        ...
        chartOptions() {
            return {
                title: {
                    text: this.$tc('nfx-analytics.view.orderSales.titleChart')
                },
                stroke: {
                    curve: 'smooth'
                },
        ...
        chartSeriesData() {
            let chartData = this.defaultChartSeriesDataWithDates(this.seriesData, 'date', 'sumNet',this.$tc('nfx-analytics.view.sumColumn'));
            if (Array.isArray(chartData)) {
                chartData.push(this.defaultChartSeriesDataWithDates(this.seriesData, 'date', 'count',this.$tc('nfx-analytics.view.countColumnShort'))[0]);
            }
        ...
        getGridColumns() {
            return [{
                ...
                label: this.$tc('nfx-analytics.view.orderSales.dateColumn'),
                allowResize: false,
                primary: true,
                inlineEdit: false,
                width: '100px'
    ...
    methods: {
        createdComponent() {
            ...
            if (this.activeStatistic.name === 'sales_all' && this.activeStatistic.pathInfo) {
