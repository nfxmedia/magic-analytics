...
use Nfx\MagicAnalytics\Components\Statistics\StatisticsHelper;

class Base
{
    const EXPORT_FILE_NAME = 'nfx-statistics.csv';
    private $config = null;
    ...
    public function getDataFromAggregations(EntitySearchResult $result, string $mainValue, string $termAggregation, ?string $entityAggregation = null): array
    {
        ...
        foreach ($aggregation->getBuckets() as $bucket)
        {
            ...
            elseif ($entityAggregation === null) {
                $name = $bucket->getKey() == 1 ? 'nfx-analytics.view.salesByAccountTypes.guest' : 'nfx-analytics.view.salesByAccountTypes.nonguest';
            }
            else {
    ...
    {
        //self::EXPORT_FILE_NAME Name des Files in /Files/plugins/nfx_modul_analytics
