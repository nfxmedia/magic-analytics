...
class CrossSelling implements StatisticsInterface
{
   public function __construct(
    ...
    public function getStatisticsData(array $parameters, Context $context): array
    {
        ...
        if (Database::tableExist('nfx_cross_selling_also_viewed', $this->connection))
        {
            $sql = "SELECT LOWER(HEX(related_product_id)) as productId, viewed
                    FROM nfx_cross_selling_also_viewed
        ...
        if (Database::tableExist('nfx_cross_selling_also_bought', $this->connection))
        {
            $sql = "SELECT LOWER(HEX(related_product_id)) as productId, sales
                    FROM nfx_cross_selling_also_bought
                    WHERE HEX(product_id) = ?
