   ...
   public function __construct(
    ...
    public function getStatisticsData(array $parameters, Context $context): array
    {
        ...
        $query = $qb
            ...
            ->from('`nfx_lexicon_entry`', 'lexicon')
            ->leftJoin('lexicon', '`nfx_lexicon_entry_translation`', 'clet',
                'lexicon.id = clet.nfx_lexicon_entry_id AND clet.language_id = UNHEX(:language)')
            ->leftJoin('lexicon', '`nfx_lexicon_entry_translation`', 'altclet',
                'lexicon.id = altclet.nfx_lexicon_entry_id AND altclet.language_id = UNHEX(:altLanguage)')
            ->setParameters([
                'language' => $languageId,
        ...
        if (!empty($parameters['salesChannelIds']))
        {
            ...
            $query->leftJoin('lexicon', '`nfx_lexicon_sales_channel`', 'clsc',
                    'lexicon.id = clsc.nfx_lexicon_entry_id AND clsc.sales_channel_id IN (:salesChannels)')
