...
use Nfx\MagicAnalytics\Components\DetectionHelper;
...
class FrontendController extends StorefrontController
{
    ...
    #[Route(path: "/widgets/nfx/analytics/visitors/{controller};{parameter1};{parameter2};{isNew}", name: "frontend.nfx.analytics.visitors", defaults: ['XmlHttpRequest' => true, '_httpCache' => false], methods: ['GET'])]
    ...
    private function updateSearchTerms(string $searchTerm, int $searchResults, string $salesChannelIdBytes, string $createdAt): void
    {
        ...
        try {
            ...
            INSERT INTO `nfx_analytics_search`
    ...
    private function updateCategoryImpressions(string $categoryId, string $salesChannelIdBytes, ?string $customerGroupIdBytes, string $deviceType, string $date, string $createdAt): void
    {
        ...
        try {
            $this->connection->executeStatement('
                INSERT INTO `nfx_analytics_category_impressions`
    ...
    private function updateProductImpressions(string $productId, string $manufacturerId, string $salesChannelIdBytes, ?string $customerGroupIdBytes, string $deviceType, string $date, string $createdAt): void
    {
        ...
        try {
            $this->connection->executeStatement('
                INSERT INTO `nfx_analytics_product_impressions`
        ...
        try {
            $this->connection->executeStatement('
                INSERT INTO `nfx_analytics_manufacturer_impressions`
    ...
    {
        $referer = $this->getDomainString($request->headers->get('nfx-referer'));
        ...
        if ($isNewVisitor == 1) {
            ...
            try {
                $this->connection->executeStatement('
                INSERT INTO `nfx_analytics_visitors`
            ...
            try {
                $this->connection->executeStatement('
                UPDATE `nfx_analytics_visitors` SET page_impressions=page_impressions+1
        ...
        if ($isNewVisitor && !empty($referer) && $referer != $host) {
            ...
            try {
                $this->connection->executeStatement('
                INSERT INTO `nfx_analytics_referer`
