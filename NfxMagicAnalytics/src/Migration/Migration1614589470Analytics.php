...
class Migration1614589470Analytics extends MigrationStep
{
    ...
    {
        if ($this->tableExist('nfx_analytics_config', $connection))
        {
            if (!$this->columnExist('nfx_analytics_config', 'plugin', $connection))
            {
                $connection->executeStatement("ALTER TABLE `nfx_analytics_config` ADD `plugin` varchar(255) DEFAULT NULL AFTER `active`;");
            }
        }

        if ($this->tableExist('nfx_analytics_groups_config', $connection))
        {
            if (!$this->columnExist('nfx_analytics_groups_config', 'plugin', $connection))
            {
                $connection->executeStatement("ALTER TABLE `nfx_analytics_groups_config` ADD `plugin` varchar(255) DEFAULT NULL AFTER `active`;");
            }
        }
    }
