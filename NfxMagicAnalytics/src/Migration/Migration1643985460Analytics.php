...
class Migration1643985460Analytics extends MigrationStep
{
    ...
    public function update(Connection $connection): void
    {
        $connection->executeStatement('
            ALTER TABLE `nfx_analytics_config` DROP COLUMN `parameter`;
        ');

        $connection->executeStatement('
            ALTER TABLE `nfx_analytics_config` ADD COLUMN `parameter` JSON NULL DEFAULT NULL AFTER `active`;
        ');
        ...
        $connection->executeStatement(
            'UPDATE `nfx_analytics_config` SET `parameter` = :hasBoth WHERE `name` IN (:stats);',
            ['hasBoth' => $hasBoth, 'stats' => $statsBoth], ['stats' => ArrayParameterType::STRING]
        );

        $connection->executeStatement(
            'UPDATE `nfx_analytics_config` SET `parameter` = :hasTable WHERE `name` IN (:stats);',
            ['hasTable' => $hasTable, 'stats' => $statsOnlyTable], ['stats' => ArrayParameterType::STRING]
        );

        $connection->executeStatement(
            'UPDATE `nfx_analytics_config` SET `parameter` = :hasNone WHERE `parameter` IS NULL;',
            ['hasNone' => $hasNone]
        );

        $allStats = $connection->executeQuery('SELECT `name`, `position` FROM `nfx_analytics_config` WHERE 1;')->fetchAllAssociative();

        foreach ($allStats as $stat)
        {
            $componentName = 'nfx-analytics-index-' . str_replace('_', '-', $stat['name']);
            $connection->executeStatement(
                'UPDATE `nfx_analytics_config`
        ...
        $connection->executeStatement("
            ...
            [
                ...
                'group_name' => 'sales',
                'label' => 'nfx-analytics.view.salesByBillingCountry.titleTree',
                'route_name' => 'nfx.analytics.getSalesByBillingCountry',
                'path_info' => '/nfx/analytics/getSalesByBillingCountry',
                'position' => 10,
                'active' => 1,
                'parameter' => '{"dashboard": {"hasTable": 1, "hasChart": 1, "showTable": 0, "showChart": 0, "position": 10}, "componentName": "nfx-analytics-index-sales-by-billing-country"}',
        ...
        $connection->executeStatement("UPDATE `nfx_analytics_config`
