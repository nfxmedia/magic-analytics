import template from './cbax-analytics-index-canceled-orders-by-month.html.twig';

const { Component, Mixin } = Shopware;

Component.register('cbax-analytics-index-canceled-orders-by-month', {
    template,
    ...
    computed: {
        ...
        chartOptions() {
            return {
                title: {
                    text: this.$tc('cbax-analytics.view.canceledOrdersByMonth.titleChart')
                },
                stroke: {
                    curve: 'smooth'
                },
        ...
        chartSeriesData() {
            return this.defaultChartSeriesData(this.seriesData, 'date', 'sum','Bestellungen', true, true);
        },
        ...
        getGridColumns() {
            return [{
                property: 'date',
                dataIndex: 'date',
                label: this.$tc('cbax-analytics.view.canceledOrdersByMonth.dateColumn'),
                allowResize: false,
                primary: true,
                inlineEdit: false,
                width: '100px'
                ...
                property: 'count',
                dataIndex: 'count',
                label: this.$tc('cbax-analytics.view.canceledOrdersByMonth.countColumn'),
                allowResize: false,
                align: 'right',
                inlineEdit: false,
                width: '100px'
                ...
                property: 'sum',
                dataIndex: 'sum',
                label: this.$tc('cbax-analytics.view.canceledOrdersByMonth.sumColumn') + this.$tc('cbax-analytics.view.' + this.grossOrNet),
                allowResize: false,
                align: 'right',
                inlineEdit: false,
                width: '100px'
    ...
    methods: {
        createdComponent() {
            ...
            if (this.activeStatistic.name === 'canceled_orders_by_month' && this.activeStatistic.pathInfo) {
