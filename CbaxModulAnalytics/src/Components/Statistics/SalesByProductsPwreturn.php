...
class SalesByProductsPwreturn implements StatisticsInterface
{
    ...
    public function getStatisticsData(array $parameters, Context $context): array
    {
        ...
        $orderNumberData = $this->getOrderNumbers($parameters, $modifiedContext, $data);

        foreach($data as &$product) {
            $product['sum'] = (int)$product['sum'];
            $product['stock'] = (int)$product['stock'];
            $product['returnSum'] = !empty($returnData[$product['id']]) ? (int)$returnData[$product['id']] : 0;
            $product['newSum'] = $product['sum'] - $product['returnSum'];
                ...
                if (!empty($prod['parentId']) && !empty($parents[$prod['parentId']])) {
                    if (empty($parentdata[$prod['parentId']])) {
                        ...
                        $parentdata[$prod['parentId']] = [
                            ...
                            'newSum' => $prod['sum'] - $prod['returnSum'],
        ...
        elseif (in_array($sortingField, ['returnSum', 'newSum', 'orderCount', 'priceAV', 'manufacturer'])) {
    ...
    private function getOrderNumbers(array $parameters, Context $context, array $data): array
    {
    ...
    {
        $sortingField = $parameters['sortBy'] ?? 'sum';
        $sortingField = in_array($sortingField, ['stock', 'number','name','sum']) ? $sortingField : 'sum';
        $direction = $parameters['sortDirection'] ?? 'DESC';
