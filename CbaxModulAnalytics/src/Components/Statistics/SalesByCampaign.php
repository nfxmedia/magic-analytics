...
class SalesByCampaign implements StatisticsInterface
{
    ...
    public function getStatisticsData(array $parameters, Context $context): array
    {
        ...
        $aggregation = $result->getAggregations()->get('sales-by-campaign');

        $data = [];
        foreach ($aggregation->getBuckets() as $bucket) {
            $sum = StatisticsHelper::calculateAmountInSystemCurrency($bucket->getResult());
            $data[] = [
                'name' => !empty($bucket->getKey()) ? $bucket->getKey() : 'cbax-analytics.view.noCampaign',
