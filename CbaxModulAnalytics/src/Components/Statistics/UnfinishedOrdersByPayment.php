...
class UnfinishedOrdersByPayment implements StatisticsInterface
{
    ...
    public function getStatisticsData(array $parameters, Context $context): array
    {
        ...
        foreach ($filteredData as $cart) {
            ...
            if (!isset($gridData[$cart['paymentMethodId']])) {
                $gridData[$cart['paymentMethodId']] = [
                ...
                $gridData[$cart['paymentMethodId']]['count']++;
                $gridData[$cart['paymentMethodId']]['gross'] += (float)$cart['grossPrice']/$currencyFactor;
                $gridData[$cart['paymentMethodId']]['net'] += (float)$cart['netPrice']/$currencyFactor;
                $gridData[$cart['paymentMethodId']]['avgGross'] = round($gridData[$cart['paymentMethodId']]['gross']/$gridData[$cart['paymentMethodId']]['count'], 2);
                $gridData[$cart['paymentMethodId']]['itemCount'] += $cart['itemCount'];
                $gridData[$cart['paymentMethodId']]['position'] += $cart['position'];
                $gridData[$cart['paymentMethodId']]['avgCount'] = round($gridData[$cart['paymentMethodId']]['itemCount']/$gridData[$cart['paymentMethodId']]['count'], 1);
            }
        }
        ...
        return ['success' => true, 'gridData' => $gridData, 'seriesData' => $gridData];
    }
}
